...
class InstructionTestSuite extends munit.FunSuite {
  ...
  test("xdv divides x by 2^combo, writes to X") {
    ...
    val res = Instructions.XDV(0, 2, 0, regs)
    assertEquals(res, 4)
  }
  ...
  test("zdv divides x by 2^combo, writes to Z") {
    ...
    val res = Instructions.ZDV(7, 3, 0, regs)
  ...
  test("zdv using reg x value") {
    ...
    // 4 would use x value
    // thus 2^3 = 8
    val res = Instructions.ZDV(7, 4, 0, regs)
    // trunc 3 / 8
    assertEquals(res, 0)
  }
}
